...
/*그래서 이게 왜 댕쩌는 거고 왜 쓰는 겁니까?
 ...
 delegate 화 하면 코드를 독립적으로 실장할 수 있게된다. -> 일일이 지시를 내리면 코드 간의 결합이 강해진다. -> 사장 코드도 바꾸고 일꾼 코드도 바꿔야함. 복잡해진다.
 delegate 패턴을 사용하면 유지보수도 편해진다.
 
 ex)
 인력시장 사장이 도배공보고 방 4개 도배하라고 함
 사장은 무슨 방을 무슨 색으로 도배를 하든 알 바가 아님 -> 사장은 일꾼에게 가서 도배를 하라는 지시까지는 함(~까라는대로 까~)
